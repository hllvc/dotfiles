# Generated by Powerlevel10k configuration wizard on 2025-11-30 at 15:23 CET.
...
() {
  ...
  # ============ WORKTREE-AWARE DIRECTORY SEGMENT ============
  # Custom 'wdir' segment that hides worktree directory from path display
  # since branch name is already shown in VCS segment

  function _p9k_worktree_get_display_path() {
    local dir="$PWD" git_path

    # Walk up to find .git file (worktree indicator)
    while [[ "$dir" != "/" ]]; do
      git_path="${dir}/.git"
      if [[ -f "$git_path" ]]; then
        # Found .git file = this is a worktree
        local git_link
  ...
  function prompt_wdir() {
    ...
    # Apply shortening: shorten non-anchor directories to 2 chars
    local -a parts=("${(@s:/:)display_path}")
    local result="" check_path="" i

    # Find anchor directories (containing marker files)
    local -a anchors=()
  ...
  function instant_prompt_wdir() {
    prompt_wdir
  }
  # ============ END WORKTREE-AWARE DIRECTORY SEGMENT ============
  ...
  typeset -g POWERLEVEL9K_DIR_HYPERLINK=false  # Disabled for worktree path override
  ...
  function my_git_formatter() {
    ...
    # Always display the current Git commit.
    res+=" %F{#585858}@${VCS_STATUS_COMMIT[1,8]}"
  ...
  # Was -1 (infinity): gitstatus counted unstaged/untracked/conflicted files on
  # every prompt for repos of ANY size. Capped so huge repos skip the dirty scan
  # (the prompt still shows branch/ahead/behind); normal repos are unaffected.
